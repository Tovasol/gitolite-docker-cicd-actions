...
if command -v apt-get >/dev/null 2>&1; then
  ...
  apt-get install -y -qq --no-install-recommends shellcheck curl unzip ca-certificates \
    || echo "  WARN: apt install failed - lint tier may skip"
fi
echo "  curl=$(command -v curl || echo NO)  unzip=$(command -v unzip || echo NO)  shellcheck=$(command -v shellcheck || echo NO)"
